...
/// Retourne la taille d'une file.
int longueur(File F) {
    if (F == NULL)printf("file existe pas - longueur");
    return (F->longueur);
}

/// Ajoute en fin de file un élément.
...
/// Fonction bfs qui pour chaque habitation actualise la distance entre chaque habitation et tous les chateaux de la map et indique si cette habitation est connexe au réseau électrique.
void bfs(GameplayScreen_t *gameplay) {

    /// Si il y a un château d'eau sur la carte.
    if (gameplay->world->chateaux->taille > 0) {
        /// On alloue un tableau dynamique de châteaux d'eau avec n (n = nb châteaux) cases.
        gameplay->bfs.tab_chateaux = malloc(sizeof(ChateauEau_t) * gameplay->world->chateaux->taille);

        /// On va parcourir la liste de châteaux d'eau.
        struct Maillon_t *parcours = gameplay->world->chateaux->premier;
        int i = 0;

        /// Parcours de toute la liste :
        while (parcours != NULL) {
            /// Le 1er château du tableau correspond au 1er château de la liste chaînée,...
            /// On copie les données nécessaires des châteaux de la liste vers les châteaux du tableau.
            gameplay->bfs.tab_chateaux[i].position = ((ChateauEau_t *) parcours->data)->position;
            gameplay->bfs.tab_chateaux[i].orientation = ((ChateauEau_t *) parcours->data)->orientation;
            i++;
            parcours = parcours->next;
        ...
        /// On cherche le nombre de routes adjacentes à chaque château.
        for (int j = 0; j < gameplay->world->chateaux->taille; j++) {
            gameplay->bfs.tab_chateaux[j].nb_routes_adjacentes = get_nb_routes_adj_batiment(gameplay,
                                                                                            gameplay->bfs.tab_chateaux[j].position,
                                                                                            gameplay->bfs.tab_chateaux[j].orientation);
        ...
        /// On actualise les routes adjacentes de chaque château qu'on stocke dans un tableau ('Vector2* routes_adjacentes').
        for (int j = 0; j < gameplay->world->chateaux->taille; j++) {
            for (int k = 0; k < gameplay->bfs.tab_chateaux[j].nb_routes_adjacentes; k++) {
                get_routes_adj_batiment(gameplay, gameplay->bfs.tab_chateaux[j].position,
                                        gameplay->bfs.tab_chateaux[j].orientation,
    ...
    /// Si il y a une centrale sur la carte.
    if (gameplay->world->centrales->taille > 0) {
        /// On alloue un tableau dynamique de centrales d'eau avec n (n = nb centrales) cases.
        ...
        /// Parcours de toute la liste de centrales :
        while (parcours3 != NULL) {
            /// La 1ère centrale du tableau correspond à la 1ère centrale de la liste chaînée,...
            /// On copie les données nécessaires des centrales de la liste vers les centrales du tableau
        ...
        /// On cherche le nombre de routes adjacentes à chaque centrale.
        for (int j = 0; j < gameplay->world->chateaux->taille; j++) {
        ...
        /// On actualise les routes adjacentes de chaque centrale qu'on stocke dans un tableau ('Vector2* routes_adjacentes').
        for (int j = 0; j < gameplay->world->chateaux->taille; j++) {
    ...
    /// Si il y a une habitation sur la carte.
    if (gameplay->world->habitations->taille > 0) {
        ...
        /// Parcours de toute la liste d'habitations :
        while (parcours2 != NULL) {
            /// La 1ère habitation du tableau correspond à la 1ère habitation de la liste chaînée,...
            /// On copie les données nécessaires des habitations de la liste vers les habitations du tableau
            /// On initialise les données utiles pour parcourir la grille à ~NULL
            gameplay->bfs.tab_habitations[z].position = ((Habitation_t *) parcours2->data)->position;
            gameplay->bfs.tab_habitations[z].orientation = ((Habitation_t *) parcours2->data)->orientation;
        ...
        /// On cherche le nombre de routes adjacentes à chaque habitation.
        for (int j = 0; j < gameplay->world->habitations->taille; j++) {
            gameplay->bfs.tab_habitations[j].nb_routes_adjacentes = get_nb_routes_adj_batiment(gameplay,
                                                                                               gameplay->bfs.tab_habitations[j].position,
                                                                                               gameplay->bfs.tab_habitations[j].orientation);
        ...
        /// On actualise les routes adjacentes de chaque habitation qu'on stocke dans un tableau ('Vector2* routes_adjacentes').
        for (int j = 0; j < gameplay->world->habitations->taille; j++) {
            for (int k = 0; k < gameplay->bfs.tab_habitations[j].nb_routes_adjacentes; k++) {
                get_routes_adj_batiment(gameplay, gameplay->bfs.tab_habitations[j].position,
                                        gameplay->bfs.tab_habitations[j].orientation,
    ...
    /// On construit la matrice de route pour pouvoir parcourir la carte à travers les routes.
    /// Chaque case si c'est une route peut être découverte ou non et possède un prédecesseur.
    for (int j = 0; j < SIM_MAP_HAUTEUR; ++j) {
        for (int k = 0; k < SIM_MAP_LARGEUR; ++k) {
            if (gameplay->world->map[j][k].type == KIND_ROUTE) {
                gameplay->bfs.matrice[j][k].route = 1;
    ...
    /// On va parcourir le tableau de chateaux d'eau.
    for (int l = 0; l < gameplay->world->chateaux->taille; l++) {
        /// Pour chaque chateau d'eau.
        for (int n = 0; n < gameplay->bfs.tab_chateaux[l].nb_routes_adjacentes; n++) {
            /// Pour chaque route adjacente à chaque chateau d'eau.
            /// On va parcourir le tableau d'habitation.
            for (int j = 0; j < gameplay->world->habitations->taille; j++) {
                /// Pour chaque habitation.
                /// Pour chaque route adjacente chaque habitation.
                for (int k = 0; k < gameplay->bfs.tab_habitations[j].nb_routes_adjacentes; k++) {
                    /// Le point de départ de notre bfs va être chaque route adjacente à chaque habitation.
                    /// Route actuelle sur laquelle on fait le parcours en largeur
                    Vector2 depart = gameplay->bfs.tab_habitations[j].routes_adjacentes[k];
                    ...
                    while (longueur(file)) {
                        ...
                        if (gameplay->bfs.matrice[(int) m.y][(int) m.x].decouverte == 0) {
                            ...
                            if (gameplay->bfs.matrice[(int) m.y][(int) m.x + 1].route == 1 &&
                                gameplay->bfs.matrice[(int) m.y][(int) m.x + 1].decouverte == 0) {
                                /// On créé une route temporaire pour manipuler plus modifier les données.
                                Vector2 temp = m;
                                /// Si la route suivante non découverte est à l'est on doit ajouter cette route dans la file.
                                temp.x = temp.x + 1;
                                /// On enfile la route à l'est.
                                enfiler(file, temp);
                                /// Si cette route n'a pas de prédécesseur.
                                if (comparateur.x == gameplay->bfs.matrice[(int) temp.y][(int) temp.x].predecesseur.x &&
                                    comparateur.y == gameplay->bfs.matrice[(int) temp.y][(int) temp.x].predecesseur.y) {
                                    /// Son prédecesseur est la route défilée (m).
                                    gameplay->bfs.matrice[(int) temp.y][(int) temp.x].predecesseur = m;
                                }
                            }
                            /// Si il y a une route non découverte à l'ouest
                            if (gameplay->bfs.matrice[(int) m.y][(int) m.x - 1].route == 1 &&
                                ...
                                /// On créé une route temporaire pour manipuler plus modifier les données.
                                Vector2 temp = m;
                                /// Si la route suivante non découverte est à l'ouest on doit ajouter cette route dans la file.
                                temp.x = temp.x - 1;
                                /// On enfile la route à l'ouest.
                                enfiler(file, temp);
                                /// Si cette route n'a pas de prédécesseur.
                                if (comparateur.x == gameplay->bfs.matrice[(int) temp.y][(int) temp.x].predecesseur.x &&
                                    comparateur.y == gameplay->bfs.matrice[(int) temp.y][(int) temp.x].predecesseur.y) {
                            ...
                            if (gameplay->bfs.matrice[(int) m.y + 1][(int) m.x].route == 1 &&
                                ...
                                /// On créé une route temporaire pour manipuler plus modifier les données.
                                Vector2 temp = m;
                                /// Si la route suivante non découverte est au sud on doit ajouter cette route dans la file.
                                temp.y = temp.y + 1;
                                /// On enfile la route au sud.
                                enfiler(file, temp);
                                /// Si cette route n'a pas de prédécesseur.
                                if (comparateur.x == gameplay->bfs.matrice[(int) temp.y][(int) temp.x].predecesseur.x &&
                                    comparateur.y == gameplay->bfs.matrice[(int) temp.y][(int) temp.x].predecesseur.y) {
                            ...
                            if (gameplay->bfs.matrice[(int) m.y - 1][(int) m.x].route == 1 &&
                                ...
                                /// On créé une route temporaire pour manipuler plus modifier les données.
                                Vector2 temp = m;
                                /// Si la route suivante non découverte est au nord on doit ajouter cette route dans la file.
                                temp.y = temp.y - 1;
                                /// On enfile la route au nord.
                                enfiler(file, temp);
                                /// Si cette route n'a pas de prédécesseur.
                                if (comparateur.x == gameplay->bfs.matrice[(int) temp.y][(int) temp.x].predecesseur.x &&
                                    comparateur.y == gameplay->bfs.matrice[(int) temp.y][(int) temp.x].predecesseur.y) {
                    ...
                    /// On vient de parcourir tout le réseau de route depuis une route adjacente à une habitation
                    /// On va désormais calculer la distance qu'il faut pour aller depuis cette route vers les châteaux.

                    /// Tableau de route pour sauvegarder toutes les routes qu'on va parcourir prédécesseur par prédécesseur
                    Vector2 tabRoute[100];
                    /// Entier qui va stocker la distance parcourue
                    int distance = 0;

                    /// On part de chaque routes adjacentes à chaque château
                    Vector2 predecesseur = gameplay->bfs.tab_chateaux[l].routes_adjacentes[n];
                    /// On stocke cette route dans le tableau
                    tabRoute[distance] = predecesseur;
                    /// On augmente la distance de 1
                    distance++;

                    /// Booléen pour arrêter de parcourir les prédécesseurs si on retombe sur une route adjacente de chaque habitation
                    bool conditionFin = false;

                    while ((predecesseur.x != depart.x || predecesseur.y != depart.y) && !conditionFin) {
                        /// Si le prédecesseur auquel on accède n'a pas de prédecesseur cad qu'on a parcouru depuis chaque château
                        /// le chemin qui amènerait vers une habitation mais vers cette habitation, il n'existe pas.
                        if (predecesseur.x == -1 && predecesseur.y == -1) {
                            /// Il n'existe pas de chemin qui relient ces deux routes
                            conditionFin = true;
                        }
                        ...
                        /// On passe au prédecesseur suivant
                        predecesseur = gameplay->bfs.matrice[(int) predecesseur.y][(int) predecesseur.x].predecesseur;
                        /// On sauvegarde ce chemin dans le tableau
                        tabRoute[distance] = predecesseur;
                        /// On augmente la distance de 1
                        distance++;
                    }
                    ...
                    if (!conditionFin) {
                        /// Si distance est intéressante cad qu'elle est inférieure à la distance déjà enregistrée
                        /// Comme on calcule chaque chemin depuis chaque route adjacente de l'habitation vers chaque route
                        /// adjacente de chaque château, on a des chemins qui se répètent et qui sont plus longs.
                        if (distance < gameplay->bfs.tab_habitations[j].distance_chateau_eau[l]) {
                            gameplay->bfs.tab_habitations[j].distance_chateau_eau[l] = distance;
                            gameplay->bfs.tab_habitations[j].position_chateau_eau[l] = gameplay->bfs.tab_chateaux[l].position;
                        }
                    ...
                    /// On réinitialise la matrice pour refaire un bfs à partir d'un autre départ.
                    for (int b = 0; b < SIM_MAP_HAUTEUR; ++b) {
                        for (int a = 0; a < SIM_MAP_LARGEUR; ++a) {
                            if (gameplay->world->map[b][a].type == KIND_ROUTE) {
                                gameplay->bfs.matrice[b][a].route = 1;
    ...
    /// On procède de même pour les centrales électriques.
...
/// Permet de transférer les données depuis les tableaux d'habitations vers les listes chaînées d'accès linéaire d'habitation.
...
/// Permet pour chaque château de créer une liste d'habitation triée par distance de la plus courte à la plus loin.
/// Utile pour la distribution des flots l'eau
